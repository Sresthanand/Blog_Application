...
exports.getUser = async (req, res, next) => {
  try {
    const user = await User.findOne({ username: req.params.uid })
      .select("cover avatar fullname username bio website")
      .lean()
      .exec();
  ...
  try {
    ...
    switch (data) {
      ...
      case "savedstory":
        ...
        data2 = await SavedStory.find({ userid: req.user.id })
          .populate({
            path: "story",
          ...
          () =>
            res
              ...
              .json({
                success: true,
                stories,
                unseennotice: req.user.unseennotice.length,
              }),
          100
        );
        break;
      case "likedstory":
        ...
        data2 = await LikedStory.find({ userid: req.user.id })
          .populate({
            path: "story",
            select: "short_des readingTime title likedBy createdAt cover user",
            populate: {
        ...
        data2.forEach(async function (t) {
          const isSaved = await SavedStory.findOne({
            story: t.story._id,
            userid: req.user.id,
          });
      ...
      case "mystory":
        ...
        user = await User.findOne({ username: req.params.uid });
        stories = await Story.find({ user: user._id })
          .select("short_des title readingTime likedBy createdAt user cover")
          .populate({
            path: "user",
            select: "username avatar",
          })
      ...
      case "readinglist":
        ...
        data2 = await ReadingList.find({ userid: req.user.id })
          .populate({
            path: "story",
            select: "short_des readingTime title likedBy createdAt cover user",
            populate: {
...
exports.postStory = async (req, res, next) => {
  try {
    ...
    const { short_des, title, html_content, charcount, topics, cover } =
      req.body;
...
exports.feed = async (req, res, next) => {
  try {
    ...
    reading_l.forEach(async function (r) {
      if (req.user) {
        const isSaved = await SavedStory.findById({
          story: r.story._id,
          userid: req.user.id,
        });
    ...
    users.forEach(function (u) {
      if (req.user) u.isFollowing = req.user.following.includes(u._id);
      data.suggestedUser.push({
        bio: u.bio,
        fullname: u.fullname,
        username: u.username,
        avatar: u.avatar,
    ...
    res
      ...
      .json({
        ...
        notices: notices.slice(
          parseInt(req.query.currIndex),
          parseInt(req.query.currIndex) + 10
        ),
      });
  } catch (e) {
    console.log(e);
  ...
  try {
    ...
    if (req.user.following.includes(uid)) {
      ...
      await Notification.deleteMany(
        { sender: req.user.username, receiver: [uid], type: "followuser" },
        (err, res) => {
          if (err) {
            console.log(err);
          }
      ...
      res
        .status(200)
        .json({ success: true, unseennotice: req.user.unseennotice.length });
    } else {
      const noti = await Notification.create({
        sender: req.user.username,
        receiver: [uid],
...
exports.addToReadingList = async (req, res, next) => {
  try {
    await ReadingList.deleteOne(
      { story: req.params.sid, userid: req.user.id },
      (err, res) => {
...
exports.getSuggestedStory = async (req, res, next) => {
  try {
    ...
    // })
    let stories = await Story.find({})
      .select("user short_des likedBy topics title cover readingTime createdAt")
      .populate({
        path: "user",
        select: "username avatar",
      })
...
exports.getSuggestedUser = async (req, res, next) => {
  ...
  try {
    ...
    if (
      !req.user ||
      (req.user &&
        req.user.following.length === 0 &&
        req.user.preferredTopics.length === 0)
    ) {
      if (req.user) {
        ...
        userst = userst.filter(function (a) {
          return (
            ...
            !a.followers.toString().includes(req.user.id)
          );
        });
      }
      ...
      for (const t of req.user.preferredTopics.reverse().slice(0, 2)) {
        ...
        if (topic) users = users.concat(topic.followedBy);
      }
      ...
      users = Array.from(new Set([...users]))
        .filter(function (a) {
          return (
            !a.followers.includes(req.user.id) &&
            a.username != req.user.username
          );
        })
        .slice(0, limit);
  ...
  try {
    ...
    if (req.query.trending) {
      topics = await Topic.find({})
        .select("name followedBy cover stories")
        .populate({
          path: "stories",
          select: "cover",
        })
    ...
    topics = topics.sort(function (a, b) {
      return (
        priority(
          a,
          req.user.following.reverse(),
          req.user.preferredTopics.reverse()
        ) -
        priority(
          b,
          req.user.following.reverse(),
