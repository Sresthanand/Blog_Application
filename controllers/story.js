...
exports.getStory = async (req, res, next) => {
  try {
    ...
    story.user.isFollowing =
      req.user && req.user.following.includes(story.user._id);
    story.likedBy = [];
...
exports.getLatestStories = async (req, res, next) => {
  try {
    ...
    stories.forEach(async function (s) {
      if (req.user) {
        s.isSaved = (await SavedStory.findOne({
          userid: req.user.id,
          story: s._id,
        }))
          ? true
...
exports.getTopicDetail = async (req, res, next) => {
  try {
    ...
    topic.relatedTopics = [];
    topic.isFollowing =
      req.user && topic.followedBy.toString().includes(req.user.id);
    topic.followedBy = [];
    topic.stories
      .sort(function (a, b) {
        return a.likedBy.length - b.likedBy.length;
    ...
    topic.stories = [];
    const related = [];
    topic.relatedTopics.forEach((x) => {
      related.push(x.name);
    });
    console.log(related);
    const relatedOnes = Array.from(new Set(related));
    console.log(relatedOnes);
    topic.relatedTopics = relatedOnes
      .filter(function (a) {
        return a != req.params.topic;
      })
      .slice(0, 5);
...
exports.toggleLike = async (req, res, next) => {
  try {
    ...
    if (story.likedBy.toString().includes(req.user.id)) {
      await LikedStory.deleteOne(
        { userid: req.user.id, story: sid },
        (err, res) => {
          if (err) {
            return next({
              statusCode: 500,
  ...
  try {
    ...
    if (req.user.id != story.user._id) {
      const noti = await Notification.create({
        ...
        avatar: `${req.user.avatar}`,
        url: `/stories/${story._id}/?commentId=${comment._id}`,
        commentId: comment._id,
        sender: req.user.id,
        story: story._id,
        Message: `${req.user.username} commented on your story`,
      });
...
exports.toggleLikeComment = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, cid, unseennotice: req.user.unseennotice.length });
  } catch (e) {
    console.log(e);
    return next({
      statusCode: 500,
...
exports.searchStory = async (req, res, next) => {
  try {
    ...
    Stories = await Story.find({
      $or: [{ title: regex }, { short_des: regex }],
    })
      .populate({
        path: "user",
        select: "username avatar",
...
exports.editStory = async (req, res, next) => {
  try {
    ...
    await Notification.deleteMany(
      { story: story._id, sender: req.user.username },
      (err, res) => {
        err && console.log(err);
      }
    );
