...
const Trending = () => {
  ...
  const { isFetching, Trendingstories, currIndex } = useSelector(
    (state) => state.trendingstory
  );
  ...
  const fetchData = () => {
    if (isFetching) dispatch(FetchTrending({ currIndex }));
  };
  ...
  const Useraction = ({ type, payload }) => {
    if (!data) {
      NotifyUser({
        content: "Please login to continue further",
        error: true,
        addToast,
      });
      history.push("/signin");
  ...
  return (
    <div className="searchpage trending">
