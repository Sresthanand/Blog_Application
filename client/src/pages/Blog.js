...
export const Modal = ({ body }) => {
  return <div className="modalwrapper">{body}</div>;
};

export const CommentComp = ({
  comment,
  toggleLikeCommentFunc,
  history,
  openModal,
}) => {
  return (
    <div className="comment_piece">
      ...
      <div>
        ...
        <div className="timestamp">
          <div
            className="like"
            onClick={() => toggleLikeCommentFunc({ cid: comment._id })}
          >
            {comment.isLiked ? (
              <FavoriteIcon className="blog_like_filled" />
            ) : (
              <FavoriteBorderIcon />
            )}
            &nbsp;&nbsp;{comment.likesCount}
          ...
          {comment.isMine && (
            <div
              onClick={() => openModal({ cid: comment._id })}
              className="comment_vert"
            >
              <MoreVertIcon />
            </div>
          )}
...
const Blog = () => {
  ...
  const { story, currIndex, ShouldFetchComments } = useSelector(
    (state) => state.story
  );
  ...
  const SubmitComment = (e) => {
    ...
    if (e.keyCode === 13 && comment.current && comment.current.value) {
      dispatch(
        addComment({
          payload: { text: comment.current.value, sid },
          callback: callback,
        })
      );
    }
  ...
  const body = (
    <div className="modalbody">
      <div className="danger" onClick={() => setModal(false)}>
        Cancel
      </div>
      {story.user.username === data?.username ? (
        <div onClick={EditStory}>Edit story</div>
      ) : (
        <div onClick={() => history.push(`/report/${story._id}`)}>
          Report story
        </div>
      )}
      {story.user.username === data?.username && (
        <div className="danger" onClick={delstoryFunc}>
          Delete story
        </div>
      )}
    </div>
      ...
      <div className="blog_author">
        ...
        <div className="blog_author_info">
          <div className="blog_author_info_1">
            <div className="blog_author_username">{story?.user?.username}</div>
          </div>
          <div className="blog_author_info_2">
            ...
            &nbsp;&nbsp;.&nbsp;&nbsp;
            <div className="blog_readTime">{story?.readingTime}</div>
          </div>
        </div>
        {!story?.isMine ? (
          <div className="actionbtn" onClick={toggleFollowUser}>
          ...
          <div className="actionbtn" onClick={EditStory}>
      ...
      <div className="blog_footer">
        <div className="blog_tag">
          {story?.topics?.map((tag) => (
            <div
              className="topic"
              onClick={() => history.push(`/topic/${tag.name}`)}
              key={tag._id}
            >
              {tag.name}
            </div>
        ...
        <div className="blog_comment">
          ...
          {story?.comments?.map((comment) => (
            <CommentComp
              comment={comment}
              key={comment._id}
              toggleLikeCommentFunc={toggleLikeCommentFunc}
              history={history}
              openModal={openModal}
            />
          ))}
