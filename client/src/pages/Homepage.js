...
const Homepage = () => {
  ...
  const { exploredstories, currIndex, isFetching } = useSelector(
    (state) => state.explore
  );
  const { data } = useSelector((state) => state.user);
  const { isFetchingfollowingstories, Followingstories, currindex } =
    useSelector((state) => state.followingstory);
  ...
  useEffect(() => {
    ...
    data?.username &&
      dispatch(FetchreadingList({ curr5Index, uid: data.username }));
  ...
  const fetchArticle = ({ type }) => {
    ...
    switch (type) {
      case 0:
        ...
        return;
      case 1:
        isFetchingfollowingstories &&
          dispatch(FetchFollowingStory({ currIndex: currindex }));
        setSelected(1);
        return;
      default:
        break;
    ...
    <div className="main">
      <div className="main_component_home">
        <div
          ...
          className="follow_people_tag"
        >
          <AddCircleIcon /> Keep up with the latest update in any topic
        </div>
        <div className="main_tabs">
          <div
            className={"tab" + (selected === 0 ? " selected-tab" : "")}
            onClick={() => fetchArticle({ type: 0 })}
          >
            Explore
          </div>
          <div
            className={"tab" + (selected === 1 ? " selected-tab" : "")}
            onClick={() => fetchArticle({ type: 1 })}
          >
            Following
          </div>
        </div>
        {selected === 0 &&
          exploredstories.map((article) => (
            <Article
              article={article}
              key={article._id}
      ...
      <div className="main_right">
        ...
        />
        {data && readingList && readingList.length > 0 && (
          <ReadingList Useraction={Useraction} readingList={readingList} />
        )}
      </div>
    </div>
  );
