...
} from "../reducers/getSearchPeople";
import {
  ClearSearchStory,
  SaveUnsaveSearchStory,
} from "../reducers/SearchStory";
...
const Searchpage = () => {
  ...
  const { isFetching, SearchedUsers, currIndex } = useSelector(
    (state) => state.searchpeople
  );
  const { isFetchingStory, Searchedstories, curr2Index } = useSelector(
    (state) => state.searchstory
  );
  ...
  return (
    <div className="searchpage">
      ...
      <FollowTabComponent
        tabstate={tabstate}
        changeTab={changeTab}
        secondTabName={"stories"}
      />
      <SearchpageContent
        tabstate={tabstate}
        term={term}
        Useraction={Useraction}
        Searchedstories={Searchedstories}
