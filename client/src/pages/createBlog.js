...
const MyComponent = ({ isEditing }) => {
  ...
  const VerifyStory = () => {
    ...
    if (images.length === 0) {
      NotifyUser({
        content: "You must add atleast one image to your story",
        type: "error",
        addToast,
      });
      return false;
    ...
    if (myEditor.current.getCharCount() < 500) {
      NotifyUser({
        content: "Your story is too short",
        type: "error",
        addToast,
      });
      return false;
  ...
  const editStoryfunc = (storydata) => {
    ...
    const { short_des, title, html_content, topics, cover, charcount } =
      storydata;
    const payload = {};
    if (short_des) {
    ...
    if (Object.keys(payload).length === 0) {
      NotifyUser({
        content: "You have not written anything yet",
        type: "error",
        addToast: addToast,
      });
      return;
        ...
        addStory({
          ...
          callback: function (res, data) {
            if (res) {
              ...
              NotifyUser({
                content: "Story could not be posted",
                type: "error",
                addToast,
              });
            }
