...
} from "../../reducers/getSuggestedTopic";
      ...
      <div style={{ display: "flex" }}>
        ...
        >
          ...
          <div className="description">
            {people.bio || "This user prefers to keep no bio"}
          </div>
        </div>
      </div>
      {!isMe && (
  ...
  return (
    <div className="topicbox" style={{ cursor: "pointer" }}>
      <div className="topicheader">
        <div
          className="topicfollowbtn"
          onClick={() =>
            Useraction({ type: "togglefollowtopic", payload: topic.name })
          }
        >
          {topic.isFollowing ? (
      ...
      <div
        ...
        onClick={() => history.push(`/topic/${topic.name}`)}
      ></div>
    </div>
  );
};
...
const FollowPagecontent = ({ tabstate }) => {
  const { isFetchingSuggestedUsers, SuggestedUsers } = useSelector(
    (state) => state.suggesteduser
  );
  const { isfetchingsuggestedtopic, suggestedtopics, currIndex2 } = useSelector(
    (state) => state.suggestedtopic
  );
  ...
  useEffect(() => {
    ...
    tabstate,
    isFetchingSuggestedUsers,
    isfetchingsuggestedtopic,
    currIndex2,
    dispatch,
  ]);

  return (
    <div className="followtabcontent">
      {tabstate === 0 &&
        SuggestedUsers.map((people) => (
          <UserCard
            ...
            history={history}
            isMe={data?.username === people.username}
          />
        ))}
      {tabstate === 1 && (
        <h3 style={{ marginTop: "-20px", marginBottom: "0px" }}>
          Topic you should follow
        </h3>
      )}
