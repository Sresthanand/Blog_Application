...
const Noticomponents = ({ notice, history }) => {
  return (
    <div
      className="notice_component"
      onClick={() => history.push(`${notice.url}`)}
    >
      <div className="notice_avatar">
        <Avatar src={notice.avatar} />
      </div>
      <div className="noti_wrapper">
        <div className="notice_text">{notice.Message}</div>
...
const Noti = () => {
  ...
  const { isFetching, notices, currIndex } = useSelector(
    (state) => state.Notice
  );
  ...
  useEffect(() => {
    if (pointer.current) {
      const max = Math.max(
        document.getElementById("notibell").offsetLeft,
        document.getElementById("notibell2").offsetLeft
      );
  ...
  return (
    <div className="notice">
      ...
      {notices.map((noti) => (
        <Noticomponents notice={noti} key={noti._id} history={history} />
      ))}
    </div>
  );
};
