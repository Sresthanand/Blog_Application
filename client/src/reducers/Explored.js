...
export const FetchExplored = createAsyncThunk(
  "user/explore",
  async ({ currIndex }) => {
    const data = await FetchData(
      `/user/getsuggestedstory?currIndex=${currIndex}`
    );
    return data;
  }
);
...
const ExploredSlice = createSlice({
  ...
  extraReducers: {
    [FetchExplored.fulfilled]: (state, action) => {
      ...
      state.exploredstories = state.exploredstories.concat(
...
export const { SaveUnsaveStoryInExplored } = ExploredSlice.actions;
