...
export const FetchMystory = createAsyncThunk(
  ...
  async ({ currindex, user }) => {
    const data = await FetchData(
      `/user/getuserdata/${user}?currIndex=${currindex}&data=mystory`
    );
    return data;
  }
);
...
const MystorySlice = createSlice({
  ...
  reducers: {
    ...
    removeFromMyStoryList(state, action) {
      state.Mystories = state.Mystories.filter(function (x) {
...
export const { SaveUnsaveMyStory, addToMyStoryList, removeFromMyStoryList } =
