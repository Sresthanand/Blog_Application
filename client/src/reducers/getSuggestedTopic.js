...
export const FetchsuggestedTopic = createAsyncThunk(
  "user/suggestedtopic",
  async ({ currIndex2 }) => {
    const data = await FetchData(
      `/user/getsuggestedtopic?currIndex=${currIndex2}`
    );
    return data;
  }
);
...
export const { FollowUnfollowSuggestedTopic } = suggestedTopicSlice.actions;
