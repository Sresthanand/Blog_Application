...
export const FetchSavedStory = createAsyncThunk(
  "user/savedstory",
  async ({ curr5Index, user }) => {
    const data = await FetchData(
      `/user/getuserdata/${user}?currIndex=${curr5Index}&data=savedstory`
    );
    return data;
  }
);
...
const SavedStorySlice = createSlice({
  ...
  reducers: {
    addToSaved(state, action) {
      state.SavedStories = [action.payload, ...state.SavedStories];
    },
...
export const { addToSaved, removeFromSaved, saveUnsaveInSavedList } =
