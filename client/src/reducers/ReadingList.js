...
export const FetchreadingList = createAsyncThunk(
  "user/readingList",
  async ({ curr3Index, user }) => {
    const data = await FetchData(
      `/user/getuserdata/${user}?currIndex=${curr3Index}&data=readinglist`
    );
    return data;
  }
);
...
const readingListSlice = createSlice({
  ...
  reducers: {
    ...
    addToreadingList(state, action) {
      state.readingList = [...action.payload, ...state.readingList];
    },
    removeFromReadingList(state, action) {
      state.readingList = state.readingList.filter(function (x) {
