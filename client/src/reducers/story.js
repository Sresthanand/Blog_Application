...
export const editStory = createAsyncThunk(
  "story/editstory",
  async ({ payload, callback }) => {
    const data = await FetchData("/story/" + payload.sid, {
      body: { payload },
      method: "PUT",
    });
...
export const deleteComment = createAsyncThunk(
  "story/deletecomment",
  async ({ payload, callback }) => {
    const data = await FetchData("/story/deletecomment/" + payload, {
      method: "DELETE",
    });
...
export const reportStory = createAsyncThunk(
  "story/reportstory",
  async ({ payload, callback }) => {
    const data = await FetchData("/story/report/" + payload.sid, {
      body: payload,
    });
...
export const fetchComments = createAsyncThunk(
  ...
  async ({ payload, callback, currIndex }) => {
    const data = await FetchData(
      "/story/fetchcomments/" + payload + "?currIndex=" + currIndex
    );
...
const StorySlice = createSlice({
  ...
  reducers: {
    ...
    togglelikeStory(state, action) {
      state.story = {
        ...state.story,
        likesCount: state.story.isLiked
    ...
    toggleFollowAuthor(state, action) {
      state.story = {
        ...
        user: {
          ...state.story.user,
          isFollowing: !state.story.user.isFollowing,
        },
      };
