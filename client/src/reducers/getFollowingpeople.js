...
export const Fetchfollowingpeople = createAsyncThunk(
  "user/followingpeople",
  async ({ currIndex, user }) => {
    const data = await FetchData(
      `/user/getuserdata/${user}?currIndex=${currIndex}&data=following`
    );
    return data;
  }
);
...
const FollowingpeopleSlice = createSlice({
  ...
  reducers: {
    ...
    addToFollowingList(state, action) {
      state.Followingpeople = [action.payload, ...state.Followingpeople];
    },
    removeFromFollowingList(state, action) {
      state.Followingpeople = state.Followingpeople.filter(function (x) {
