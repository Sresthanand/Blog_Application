...
export const FetchLikedstory = createAsyncThunk(
  "user/likedstory",
  async ({ curr4Index, user }) => {
    const data = await FetchData(
      `/user/getuserdata/${user}?currIndex=${curr4Index}&data=likedstory`
    );
    return data;
  }
);
...
const LikedstorySlice = createSlice({
  ...
  reducers: {
    ...
    addToLikedList(state, action) {
      state.Likedstories = [action.payload, ...state.Likedstories];
    },
    removeFromLikedList(state, action) {
      state.Likedstories = state.Likedstories.filter(function (x) {
...
export const { SaveUnsaveLikedStory, addToLikedList, removeFromLikedList } =
